...
file_path = fr'C:\Users\Andres.Acosta\Downloads\iSeries Turnover {date_str}.docx'
print(file_path)
                        ...
                        if row_idx < len(target_table_2.rows):
                            ...
                            if i + counter < len(row2.cells):
                                ...
                                for paragraph in row2.cells[i + counter].paragraphs:
                                    ...
                                    for run in paragraph.runs:
                                        run.bold = True
                                print(f"Filled out matching row in target_table_2 at Row {row_idx}.")
                        else:
                            print(f"⚠️ Warning: target_table_2 does not have a Row {row_idx}.")
...
def wait_until_next_sharp_hour():
    ...
    print(f"💤💤💤💤 Sleeping for {delay/60:.1f} minutes until {next_hour.strftime('%H:%M')}...")
    time.sleep(delay)
